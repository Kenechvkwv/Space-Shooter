class Entity extends Phaser.GameObjects.Sprite {
  ...
  explode(canDestroy) {
    if (!this.getData("isDead")) {
      // Sets texture to the explosion image, then plays the animation
      this.setTexture("sprExplosion");
      this.play("sprExplosion"); // play the animation
      // pick a random explosion sound within the array defined in this.sfx in SceneMain
  ...
  update() {
    ...
    if (this.getData("isShooting")) {
      if (this.getData("timerShootTick") < this.getData("timerShootDelay")) {
        this.setData("timerShootTick", this.getData("timerShootTick") + 1); // every game update, increase timerShootTick by one
      } else {
...
class ScrollingBackground {
  ...
  createLayers() {
    for (var i = 0; i < 2; i++) {
      // Boss Chux's theory of game illusion
